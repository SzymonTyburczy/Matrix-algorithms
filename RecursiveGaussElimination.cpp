...
std::vector<double> solve_recursive_gauss(Matrix A, std::vector<double> b){
    ...
    if (n==0) return {};
    if (n==1){
        if (fabs(A[0][0]) < EPS) throw runtime_error("Zero pivot in base case");
    ...
    if (maxv < EPS) throw std::runtime_error("Matrix is singular (pivot ~ 0)");
    ...
    for (int i=1;i<n;i++){
        for (int j=1;j<n;j++) A_sub[i-1][j-1] = A[i][j];
        b_sub[i-1] = b[i];
    }
    ...
    double s = 0.0;
    for (int j=1;j<n;j++) s += A[0][j] * x_sub[j-1];
    x[0] = (b[0] - s) / A[0][0];
    for (int i=1;i<n;i++) x[i] = x_sub[i-1];
    return x;
}
